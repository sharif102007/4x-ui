...
install_base() {
    ...
    case "${release}" in
        ...
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
            dnf -y makecache && dnf install -y -q cronie curl tar tzdata socat ca-certificates openssl nftables iproute
            ;;
        centos)
            if [[ "${VERSION_ID}" =~ ^7 ]]; then
                yum -y makecache && yum install -y -q cronie curl tar tzdata socat ca-certificates openssl nftables iproute
            else
                dnf -y makecache && dnf install -y -q cronie curl tar tzdata socat ca-certificates openssl nftables iproute
            fi
            ;;
        arch | manjaro | parch)
            ...
            pacman -Syu --noconfirm cronie curl tar tzdata socat ca-certificates openssl nftables iproute2
            ;;
        ...
        alpine)
            ...
            ;;
        *)
            apt-get update && apt install -y -q cron curl tar tzdata socat ca-certificates openssl nftables iproute2
            ;;
    esac

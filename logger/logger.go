...
// logger is never nil. It starts out as go-logging's default (stderr) logger and
// InitLogger replaces it with the configured multi-backend one. It was previously
// nil until InitLogger ran, so every exported function below dereferenced a nil
// pointer on any code path that logged before initialization - including
// everything reached under `go test`, which never calls InitLogger.
var logger = logging.MustGetLogger("x-ui")

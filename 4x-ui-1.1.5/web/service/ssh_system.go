...
func (s sshSystem) writeStunnel(svcs []stunnelSvc) error {
	...
	// Apply latency/stability options to both the accepted client socket (l)
	// and the loopback OpenSSH/payload-gateway socket (r). These are stunnel
	// socket options, not Linux sysctl keys.
	sb.WriteString("socket = l:TCP_NODELAY=1\n")
	sb.WriteString("socket = r:TCP_NODELAY=1\n")
	sb.WriteString("socket = l:SO_KEEPALIVE=1\n")
	sb.WriteString("socket = r:SO_KEEPALIVE=1\n\n")
	...
	configData := []byte(sb.String())
	configChanged := true
	if current, readErr := os.ReadFile(stunnelConfPath); readErr == nil && bytes.Equal(current, configData) {
		configChanged = false
	}
	if configChanged {
		if err := os.WriteFile(stunnelConfPath, configData, 0o644); err != nil {
			return err
		}
	}

	// Dedicated systemd unit running our config in the foreground. It is
	// intentionally separate from Debian's stunnel4.service; the panel owns
	// only xui-stunnel.service.
...
`, stunnelBin, stunnelConfPath)
	unitData := []byte(unit)
	unitChanged := true
	if current, readErr := os.ReadFile(stunnelUnitPath); readErr == nil && bytes.Equal(current, unitData) {
		unitChanged = false
	}
	if unitChanged {
		if err := os.WriteFile(stunnelUnitPath, unitData, 0o644); err != nil {
			return err
		}
	}
	systemctl, err := s.tool("systemctl", "/usr/bin/systemctl", "/bin/systemctl")
	...
	active, _ := s.run(systemctl, "is-active", "xui-stunnel.service")
	if configChanged || unitChanged || strings.TrimSpace(active) != "active" {
		if out, restartErr := s.run(systemctl, "restart", "xui-stunnel.service"); restartErr != nil {
		...
		logger.Debugf("ssh-manager: stunnel already current with %d service(s); connections preserved", len(svcs))
	}
	return nil
}

...
//
// Every failure here is logged and swallowed. The nftables policer is active
// while HTB is being prepared and remains the fallback on unsupported hosts.
// Once HTB/IFB is healthy, the script removes only the policer rules so the
// same flow is never queued and dropped at the same configured rate.

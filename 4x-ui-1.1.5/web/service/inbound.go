...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if err := s.checkExternalPortConflict(inbound.Port); err != nil {
		return inbound, false, err
	}
...
func (s *InboundService) DelInbound(id int) (bool, error) {
	...
	if err := tx.Where("inbound_id = ?", id).Delete(xray.ClientTraffic{}).Error; err != nil {
		return false, err
	}
	...
	if len(clients) > 0 {
		...
		for _, batch := range chunkStrings(uniqueNonEmptyStrings(emails), sqliteMaxVars) {
			if err := tx.Where("client_email IN ?", batch).
	...
	if inbound.Enable {
		...
		if err := s.xrayApi.DelInbound(inbound.Tag); err == nil {
			...
			logger.Debug("Unable to delete inbound by api:", err)
			needRestart = true
		}
		s.xrayApi.Close()
	}
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	speedPolicyChanged := s.speedLimitConfigChanged(oldInbound, inbound)
	...
	needRestart := false
	if speedPolicyChanged {
		// Generated per-client Xray outbounds and nftables rules are rebuilt by
		// GetXrayConfig, not by the incremental inbound API.
		needRestart = true
	}
	s.xrayApi.Init(p.GetAPIPort())
	...
	if inbound.Enable {
		runtimeInbound, err2 := s.buildRuntimeInboundForAPI(db, oldInbound)
		if err2 != nil {
	...
	openInboundFirewall(oldInbound)
...
func (s *InboundService) AddInboundClient(data *model.Inbound) (bool, error) {
	...
	if !ok || len(interfaceClients) == 0 {
		return false, common.NewError("clients payload is empty or invalid")
	}
	...
	if !ok {
		return false, common.NewError("stored inbound clients are invalid")
	}
	...
	needRestart := false
	// Speed-limited clients are represented by generated Xray outbounds and
	// routing rules. The Xray API AddUser endpoint only updates the inbound's
	// client list, so a full config restart is required when a new limited
	// client is added.
	for _, client := range clients {
		if client.SpeedLimit && (client.DownloadMbps > 0 || client.UploadMbps > 0) {
			needRestart = true
			break
		}
	}
		...
		if len(client.Email) > 0 {
			if client.Enable {
				...
				if oldInbound.Protocol == "shadowsocks" {
					var valid bool
					cipher, valid = oldSettings["method"].(string)
					if !valid || cipher == "" {
						needRestart = true
						continue
					}
...
func (s *InboundService) speedLimitConfigChanged(oldInbound, newInbound *model.Inbound) bool {
	oldClients, oldErr := s.GetClients(oldInbound)
	newClients, newErr := s.GetClients(newInbound)
	if oldErr != nil || newErr != nil {
		return true
	}
	state := func(clients []model.Client) []string {
		values := make([]string, 0, len(clients))
		for _, client := range clients {
			key := client.Email
			if key == "" {
				key = client.ID + ":" + client.Password + ":" + client.Auth
			}
			values = append(values, fmt.Sprintf("%s:%t:%d:%d", key, client.SpeedLimit, client.DownloadMbps, client.UploadMbps))
		}
		sort.Strings(values)
		return values
	}
	return strings.Join(state(oldClients), "|") != strings.Join(state(newClients), "|")
}
...
func (s *InboundService) DelInboundClient(inboundId int, clientId string) (bool, error) {
	...
	needApiDel := false
	clientFound := false
	removedSpeedLimit := false
	for _, client := range interfaceClients {
		c, ok := client.(map[string]any)
		if !ok {
			return false, common.NewError("stored inbound client entry is invalid")
		}
		cID, ok := c[client_key].(string)
		if !ok || cID == "" {
			return false, common.NewError("stored inbound client ID is invalid")
		}
		if cID == clientId {
			clientFound = true
			email, _ = c["email"].(string)
			needApiDel, _ = c["enable"].(bool)
			removedSpeedLimit, _ = c["speedLimit"].(bool)
		} else {
	...
	needRestart := false
	if removedSpeedLimit {
		// Removing a speed-limited client must rebuild generated Xray routing
		// and nftables state; RemoveUser alone cannot remove those objects.
		needRestart = true
	}
...
func (s *InboundService) UpdateInboundClient(data *model.Inbound, clientId string) (bool, error) {
	...
	if len(clients) != 1 {
		return false, common.NewError("exactly one client is required for update")
	}
	...
	interfaceClients, ok := settings["clients"].([]any)
	if !ok || len(interfaceClients) != 1 {
		return false, common.NewError("exactly one valid client payload is required")
	}
	...
	settingsClients, ok := oldSettings["clients"].([]any)
	if !ok || clientIndex >= len(settingsClients) {
		return false, common.NewError("stored inbound clients are invalid")
	}
	...
	if clientIndex >= 0 && clientIndex < len(settingsClients) {
		if oldMap, ok := settingsClients[clientIndex].(map[string]any); ok {
			// Keep quota fields when an older frontend omits them from the
			// client payload. Explicit false/zero values are still respected.
			if newMap, isNewMap := interfaceClients[0].(map[string]any); isNewMap {
				for _, key := range []string{"speedLimit", "downloadMbps", "uploadMbps"} {
	...
	needRestart := false
	oldClient := oldClients[clientIndex]
	if oldClient.SpeedLimit != clients[0].SpeedLimit ||
		oldClient.DownloadMbps != clients[0].DownloadMbps ||
		oldClient.UploadMbps != clients[0].UploadMbps ||
		oldEmail != clients[0].Email {
		// The incremental Xray user API cannot update generated speed-limit
		// outbounds/routes. Force the normal config rebuild for these changes.
		needRestart = true
	}
	if err := tx.Save(oldInbound).Error; err != nil {
		return false, err
	...
	if len(oldEmail) > 0 {
		...
		if clients[0].Enable {
			...
			if oldInbound.Protocol == "shadowsocks" {
				...
				cipher, valid = oldSettings["method"].(string)
				if !valid || cipher == "" {
					logger.Debug("Invalid shadowsocks method in stored inbound settings")
					needRestart = true
					s.xrayApi.Close()
					return needRestart, nil
				}
...
func (s *InboundService) AddTraffic(inboundTraffics []*xray.Traffic, clientTraffics []*xray.ClientTraffic) (bool, bool, error) {
	...
	needRestart1, count, clientsErr := s.disableInvalidClients(tx)
	if clientsErr != nil {
		logger.Warning("Error in disabling invalid clients:", clientsErr)
	} else if count > 0 {
		logger.Debugf("%v clients disabled", count)
		disabledClientsCount = count
	}

	needRestart2, count, inboundsErr := s.disableInvalidInbounds(tx)
	if inboundsErr != nil {
		logger.Warning("Error in disabling invalid inbounds:", inboundsErr)
	} else if count > 0 {

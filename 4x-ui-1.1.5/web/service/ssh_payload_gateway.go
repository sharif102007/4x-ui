...
// tuneTunnelSocket keeps the small interactive packets used by SSH and
// tunneled games out of Nagle/delayed-write queues. Go enables TCP_NODELAY on
// TCP connections by default, but setting it explicitly here documents and
// protects the latency contract if a connection is ever supplied by another
// listener implementation. Keepalive is for connection stability; it does not
// replace SSH's ClientAlive probes.
func tuneTunnelSocket(conn net.Conn) {
...
func (g *payloadGateway) handle(client net.Conn) {
	...
	dialer := net.Dialer{Timeout: dialTimeout, KeepAlive: tcpKeepAlivePeriod}
	backend, err := dialer.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", g.backend))
	if err != nil {
		logger.Warningf("ssh-manager: gateway #%d backend dial failed: %v", g.id, err)
		return
	}
	defer backend.Close()
	tuneTunnelSocket(backend)

...
arch() {
    case "$(uname -m)" in
        ...
        *) echo -e "${red}Unsupported CPU architecture: $(uname -m). This release supports Linux x86_64/amd64 only.${plain}" >&2; exit 1 ;;
    esac
}
...
verify_release_archive() {
    ...
    checksum_text=$(curl -fLsS --connect-timeout 15 --max-time 120 --retry 3 "${checksum_url}") || {
        echo -e "${red}Failed to download the release checksum.${plain}"
        return 1
    }
    expected=$(printf '%s\n' "${checksum_text}" | awk 'NR == 1 {print tolower($1)}')
    if [[ ! "${expected}" =~ ^[0-9a-f]{64}$ ]]; then
        echo -e "${red}The release checksum is invalid.${plain}"
        return 1
    fi
    ...
    if [[ "${actual}" != "${expected}" ]]; then
        echo -e "${red}Release checksum mismatch; the current installation was not changed.${plain}"
        return 1
    fi
    tar -tzf "${archive}" > /dev/null 2>&1 || {
        echo -e "${red}The downloaded release archive is corrupt.${plain}"
        return 1
    }
    for member in x-ui/x-ui x-ui/x-ui.sh x-ui/bin/xray-linux-$(arch); do
        tar -tzf "${archive}" | grep -Fxq "${member}" || {
            echo -e "${red}Release archive is missing ${member}.${plain}"
            return 1
        }
    done
    if tar -tzf "${archive}" | grep -Eq '(^/|(^|/)\.\.(/|$))'; then
        echo -e "${red}Unsafe path found in the release archive.${plain}"
        return 1
    fi
...
install_base() {
    case "${release}" in
        ubuntu | debian | armbian)
            ...
            apt-get install -y -q cron curl tar tzdata socat ca-certificates openssl nftables iptables iproute2 stunnel4 ||
                echo -e "${yellow}Warning: some optional dependencies could not be installed.${plain}"
            ;;
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
            dnf -y makecache && dnf install -y -q cronie curl tar tzdata socat ca-certificates openssl nftables iptables iproute stunnel
            ;;
        centos)
            if [[ "${VERSION_ID}" =~ ^7 ]]; then
                yum -y makecache && yum install -y cronie curl tar tzdata socat ca-certificates openssl nftables iptables iproute stunnel
            else
                dnf -y makecache && dnf install -y -q cronie curl tar tzdata socat ca-certificates openssl nftables iptables iproute stunnel
            fi
            ;;
        arch | manjaro | parch)
            pacman -Syu --noconfirm cronie curl tar tzdata socat ca-certificates openssl nftables iptables iproute2 stunnel
            ;;
        opensuse-tumbleweed | opensuse-leap)
            zypper refresh && zypper -q install -y cron curl tar timezone socat ca-certificates openssl nftables iptables iproute2 stunnel
            ;;
        alpine)
            apk update && apk add dcron curl tar tzdata socat ca-certificates openssl nftables iptables iproute2 stunnel
            ;;
        *)
            apt-get update || echo -e "${yellow}Warning: apt index refresh reported an error; trying the available package indexes.${plain}"
            apt-get install -y -q cron curl tar tzdata socat ca-certificates openssl nftables iptables iproute2 stunnel4 ||
...
# Fail2Ban IP-limit jail.
#
# This used to be a menu-only step, so a fresh install had Xray access logging
# on but nothing acting on it - the IP limit set per client in the panel was
# never actually enforced at the firewall. Setting it up here makes a clean
# install match what the panel UI implies.
#
# Runs `x-ui iplimit install` as a separate process on purpose: that code path
# calls `exit 1` when a package install fails, and a missing Fail2Ban must never
# abort a panel installation that is otherwise fine. Skipped when the jail is
# already present so re-running the installer is idempotent.
setup_fail2ban() {
    if [[ -f /etc/fail2ban/jail.d/3x-ipl.conf ]]; then
        echo -e "${green}Fail2Ban IP-limit jail already configured, skipping.${plain}"
        return 0
    fi
    if [[ ! -x /usr/bin/x-ui ]]; then
        return 0
    fi
    echo -e "${yellow}Setting up Fail2Ban IP limit...${plain}"
    if /usr/bin/x-ui iplimit install; then
        echo -e "${green}Fail2Ban IP limit configured.${plain}"
    else
        echo -e "${yellow}Fail2Ban setup did not complete. The panel is installed and working;${plain}"
        echo -e "${yellow}run 'x-ui iplimit install' later to enable per-client IP limiting.${plain}"
    fi
    return 0
}
...
prompt_and_setup_ssl() {
    ...
    # Every branch below waits on `read`. With no terminal attached - piped input,
    # cron, an automation script - that wait never ends and the installer looks
    # frozen. Bail out with instructions instead.
    if [[ ! -t 0 ]]; then
        echo -e "${yellow}Not running interactively - skipping SSL certificate setup.${plain}"
        echo -e "${yellow}Configure it later by running: x-ui${plain}"
        return 1
    fi
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls --connect-timeout 10 --max-time 30 --retry 3 "https://api.github.com/repos/sharif102007/4x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls --connect-timeout 10 --max-time 30 --retry 3 "https://api.github.com/repos/sharif102007/4x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLR --connect-timeout 15 --max-time 300 --retry 3 --retry-delay 2 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/sharif102007/4x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        # 4x-ui versions its own releases from 1.0.0. The inherited 2.3.5 floor
        # came from upstream 3x-ui and rejected every valid 4x-ui tag when a
        # version was pinned explicitly.
        min_version="1.0.0"
        ...
        url="https://github.com/sharif102007/4x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
        echo -e "Beginning to install x-ui $1"
        curl -4fLR --connect-timeout 15 --max-time 300 --retry 3 --retry-delay 2 -o ${xui_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download x-ui $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    if ! verify_release_archive "x-ui-linux-$(arch).tar.gz" "${tag_version}"; then
    ...
    # Helper scripts shipped in the release tarball. Older tarballs predate them,
    # so fetch as a fallback rather than failing the install.
    for helper in network-optimize.sh limits-diag.sh 4xui-shaper.sh; do
        if [[ ! -f "${helper}" ]]; then
            curl -4fLRo "${helper}" "https://raw.githubusercontent.com/sharif102007/4x-ui/${tag_version}/${helper}" ||
                echo -e "${yellow}Warning: could not fetch ${helper}; 'x-ui netopt' and 'x-ui diag' will be unavailable${plain}"
        fi
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/sharif102007/4x-ui/${tag_version}/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/sharif102007/4x-ui/${tag_version}/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/sharif102007/4x-ui/${tag_version}/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/sharif102007/4x-ui/${tag_version}/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac

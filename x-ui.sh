...
# Apply conservative host tuning for long-lived Xray/SSH connections. The
# profile is intentionally small and reversible: it raises connection/file
# limits, enables TCP keepalive, reduces TIME_WAIT pressure, and avoids using
# swap aggressively on small VPS instances. Existing BBR settings are kept.
optimize_system() {
    if [[ "${release}" == "alpine" || -d /etc/sysctl.d ]]; then
        mkdir -p /etc/sysctl.d
        cat > /etc/sysctl.d/99-xui-performance.conf <<'EOF'
# Managed by 4x-ui. Safe defaults for Xray and SSH tunnels.
fs.file-max = 2097152
...
EOF
    ...
    # The drop-in also updates already-installed services without requiring a
    # full reinstall. It prevents descriptor/task limits from becoming the
    # bottleneck before RAM or CPU does.
    if command -v systemctl >/dev/null 2>&1 && [[ -d /etc/systemd/system ]]; then
        mkdir -p /etc/systemd/system/x-ui.service.d
        cat > /etc/systemd/system/x-ui.service.d/10-performance.conf <<'EOF'
[Service]
LimitNOFILE=1048576
LimitNPROC=65535
TasksMax=infinity
...
cleanup_ssh_manager() {
    ...
    if [[ -f /etc/pam.d/sshd ]]; then
        sed -i \
            -e '/# 4x-ui SSH concurrent-session limit/d' \
            -e '\|auth requisite pam_exec.so quiet /usr/local/x-ui/bin/ssh-session-limit|d' \
            /etc/pam.d/sshd 2>/dev/null || true
    fi
    rm -f /etc/security/4xui-ssh-session-limits.conf
    rm -f /usr/local/x-ui/bin/ssh-session-limit
...
uninstall() {
    ...
    rm -f /etc/sysctl.d/99-xui-performance.conf
    rm -f /etc/systemd/system/x-ui.service.d/10-performance.conf
    if command -v sysctl >/dev/null 2>&1; then
        sysctl --system >/dev/null 2>&1 || true
    fi
    if command -v systemctl >/dev/null 2>&1; then
...
# Non-interactive dependency/bootstrap path used by the panel when an Xray
# client IP or concurrent-session limit is first enabled. Keep this separate
# from install_iplimit because the latter is an interactive menu action.
ensure_iplimit_auto() {
    if [[ $EUID -ne 0 ]]; then
        LOGE "Automatic IP/session-limit setup requires root privileges"
        return 1
    fi

    local packages="fail2ban nftables iptables iproute2"
    if ! command -v fail2ban-client >/dev/null 2>&1; then
        LOGI "Installing Fail2Ban and firewall dependencies for Xray limits..."
        case "${release}" in
            ubuntu | debian | armbian)
                apt-get update
                ...
                ;;
            fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
                packages="fail2ban nftables iptables iproute"
            ...
            centos)
                packages="fail2ban nftables iptables iproute"
                if command -v dnf >/dev/null 2>&1 && [[ ! "${VERSION_ID}" =~ ^7 ]]; then
                    dnf install -y ${packages}
                else
                    yum install -y ${packages}
                fi
            ...
            opensuse-tumbleweed | opensuse-leap)
                ...
                ;;
            *)
                LOGE "Unsupported operating system (${release}); cannot auto-install Fail2Ban"
                return 1
                ;;
        esac
    fi
    ...
    if ! command -v fail2ban-client >/dev/null 2>&1; then
        LOGE "Fail2Ban installation failed"
        return 1
    fi

    mkdir -p "${log_folder}" /etc/fail2ban/jail.d /etc/fail2ban/filter.d /etc/fail2ban/action.d
    touch "${iplimit_log_path}" "${iplimit_banned_log_path}"
    iplimit_remove_conflicts
    create_iplimit_jails
    ...
    if [[ "${release}" == "alpine" ]]; then
        ...
        service fail2ban restart >/dev/null 2>&1 || service fail2ban start >/dev/null 2>&1
    fi

    if fail2ban-client status 3x-ipl >/dev/null 2>&1; then
        LOGI "Fail2Ban Xray limit jail is active"
        return 0
    fi

    LOGE "Fail2Ban was installed but the 3x-ipl jail could not be activated"
    return 1
}
...
│  ${blue}x-ui optimize-system${plain}        - Apply RAM/CPU/network tuning     │
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "restart-xray")
            ...
            ;;
        "optimize-system")
            check_install 0 && optimize_system
            ;;
        "status")

...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/sharif102007/4x-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/sharif102007/4x-ui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/sharif102007/4x-ui/v$tag_version/install.sh") v$tag_version"
...
cleanup_ssh_manager() {
    local group="xui-ssh-users"
    local members=""
    local user=""

    echo "Cleaning SSH Manager users and host integrations..."

    # Every account created by SSH Manager is placed in this dedicated group.
    # Read the group before removing it; never scan or delete unrelated system users.
    if command -v getent >/dev/null 2>&1; then
        members=$(getent group "$group" | awk -F: '{print $4}')
    fi
    if [[ -n "$members" ]]; then
        IFS=',' read -ra managed_users <<< "$members"
        for user in "${managed_users[@]}"; do
            [[ -z "$user" ]] && continue
            case "$user" in
                root|daemon|bin|sys|sync|games|man|lp|mail|news|uucp|proxy|www-data|backup|list|irc|gnats|nobody|_apt|systemd-*|messagebus|sshd)
                    echo "Skipping protected account: $user"
                    continue
                    ;;
            esac
    ...
    rm -f /etc/sysctl.d/99-xui-ssh-manager.conf
    command -v systemctl >/dev/null 2>&1 && systemctl daemon-reload 2>/dev/null || true

    if command -v nft >/dev/null 2>&1; then
        nft delete table inet fourxui_ssh 2>/dev/null || true
        nft delete table inet fourxui_xray 2>/dev/null || true
    fi
    ...
    command -v systemctl >/dev/null 2>&1 && systemctl enable --now ssh.socket 2>/dev/null || true
}

# network_optimize / limits_diag delegate to the standalone scripts installed
# alongside the binary. Keeping the logic out of this file means both can be
# syntax-checked and run on their own, and neither can break panel management if
# it fails.
network_optimize() {
    local action="${1:-status}"
    local script="${xui_folder}/network-optimize.sh"
    if [[ ! -x "${script}" ]]; then
        echo -e "${red}network-optimize.sh not found at ${script}${plain}"
        echo -e "${yellow}Reinstall or update the panel to install it.${plain}"
        return 1
    fi
    case "${action}" in
        apply | rollback | status) ;;
        *)
            echo -e "${red}usage: x-ui netopt {apply|rollback|status}${plain}"
            return 1
            ;;
    esac
    "${script}" "${action}"
...
shaper() {
    local action="${1:-status}"
    local script="${xui_folder}/4xui-shaper.sh"
    if [[ ! -x "${script}" ]]; then
        echo -e "${red}4xui-shaper.sh not found at ${script}${plain}"
        echo -e "${yellow}Reinstall or update the panel to install it.${plain}"
        return 1
    fi
    case "${action}" in
        apply | rollback | status | check) ;;
        *)
            echo -e "${red}usage: x-ui shaper {apply|rollback|status|check} [interface]${plain}"
            return 1
            ;;
    esac
    "${script}" "${action}" "${2:-}"
...
limits_diag() {
    local action="${1:-full}"
    local script="${xui_folder}/limits-diag.sh"
    if [[ ! -x "${script}" ]]; then
        echo -e "${red}limits-diag.sh not found at ${script}${plain}"
        echo -e "${yellow}Reinstall or update the panel to install it.${plain}"
        return 1
    fi
    case "${action}" in
        full | counters | latency | udp) ;;
        *)
            echo -e "${red}usage: x-ui diag {full|counters|latency|udp [peer] [secs]}${plain}"
            return 1
            ;;
    esac
    # udp takes an optional peer host and duration; the other modes ignore them.
    "${script}" "${action}" "${2:-}" "${3:-}"
}
...
uninstall() {
    ...
    # Remove tc/IFB shaping while the script that knows how to do it still exists.
    if [[ -x "${xui_folder}/4xui-shaper.sh" ]]; then
        "${xui_folder}/4xui-shaper.sh" rollback || true
    fi

    # Restore the sysctl values this panel changed before its own files (and the
    # backup that makes the restore possible) are deleted.
    if [[ -x "${xui_folder}/network-optimize.sh" ]]; then
        "${xui_folder}/network-optimize.sh" rollback || true
    else
        rm -f /etc/sysctl.d/99-4xui-network.conf
    fi
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/sharif102007/4x-ui/main/install.sh)${plain}"
    echo ""
...
update_shell() {
    curl -fLRo /usr/bin/x-ui -z /usr/bin/x-ui https://github.com/sharif102007/4x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
# ---------------------------------------------------------------------------
#
# ssl_cert_issue is interactive by design (it asks for the domain, the port and
# the reload command), so nothing could ever run it unattended - which is why
# "auto SSL" never actually happened and inbounds kept pointing at certificate
# files that were never created.
#
# The functions below are the non-interactive path:
#
#   x-ui ssl auto <domain>   issue/renew one domain, no prompts
#   x-ui ssl fix             find every missing certificate referenced by an
#                            Xray inbound and issue it
#   x-ui ssl renew           force-renew everything acme.sh knows about
#
# Challenge selection is automatic:
#   * Cloudflare credentials present (CF_Token or CF_Key in the environment or
#     already saved in acme.sh)         -> DNS-01, needs no open port at all
#   * otherwise                          -> standalone HTTP-01 on port 80
#
# Port 80 is the reason HTTP-01 kept failing here: the SSH Manager's payload
# inbound listens on it from inside the panel process. When that is the case we
# stop the panel, issue, and restart it - with a trap so the panel is restarted
# even if acme.sh fails or the operator interrupts. If something *else* owns
# port 80 we refuse rather than killing an unknown service.

acme_bin() { echo "${HOME}/.acme.sh/acme.sh"; }
...
ensure_acme_cron() {
    # acme.sh installs its own cron entry, but a panel restored from a backup or
    # copied between hosts can end up without it - and a missing cron is
    # invisible until the certificate silently expires.
    if crontab -l 2> /dev/null | grep -q 'acme.sh --cron'; then
        return 0
    fi
    LOGW "acme.sh renewal cron missing, reinstalling it"
    "$(acme_bin)" --install-cronjob > /dev/null 2>&1 || true
}

# Who is listening on port 80? Echoes "free", "panel" or the offending process.
port80_owner() {
    local out
    out=$(ss -ltnp 2> /dev/null | awk '$4 ~ /:80$/ {print $NF}' | head -1)
    if [[ -z "${out}" ]]; then
        echo "free"
        return
    fi
    if [[ "${out}" == *"x-ui"* || "${out}" == *"sshd"* || "${out}" == *"stunnel"* ]]; then
        echo "panel"
        return
    fi
    echo "${out}"
...
ssl_cert_auto() {
    local domain="${1:-}"
    if [[ -z "${domain}" ]]; then
        LOGE "usage: x-ui ssl auto <domain>"
        return 1
    fi
    if ! is_domain "${domain}"; then
        LOGE "Invalid domain: ${domain}"
        return 1
    fi

    ensure_acme || { LOGE "acme.sh could not be installed"; return 1; }

    # Destinations default to the panel's own layout, but ssl_cert_fix passes
    # the paths the Xray config actually references. Issuing into
    # /root/cert/<domain>/ while the inbound reads from somewhere else looks
    # like success and fixes nothing.
    local acme certPath reloadCmd fullchain keyfile
    acme="$(acme_bin)"
    certPath="/root/cert/${domain}"
    fullchain="${2:-${certPath}/fullchain.pem}"
    keyfile="${3:-${certPath}/privkey.pem}"
    mkdir -p "$(dirname "${fullchain}")" "$(dirname "${keyfile}")"
    reloadCmd="systemctl restart x-ui 2>/dev/null || rc-service x-ui restart 2>/dev/null"

    "${acme}" --set-default-ca --server letsencrypt > /dev/null 2>&1

    local issued=1
    if have_cf_credentials; then
        LOGI "Cloudflare credentials found, using DNS-01 (no open port required)"
        "${acme}" --issue --dns dns_cf -d "${domain}" && issued=0
    else
        local owner
        owner="$(port80_owner)"
        case "${owner}" in
            free)
                LOGI "Port 80 is free, using standalone HTTP-01"
                "${acme}" --issue -d "${domain}" --standalone --httpport 80 && issued=0
                ;;
            panel)
                LOGW "Port 80 is held by the panel; stopping it for the challenge"
                # Restart the panel no matter how this function exits.
                trap 'systemctl start x-ui 2>/dev/null || rc-service x-ui start 2>/dev/null' RETURN
                systemctl stop x-ui 2> /dev/null || rc-service x-ui stop 2> /dev/null
                sleep 2
                "${acme}" --issue -d "${domain}" --standalone --httpport 80 && issued=0
                ;;
            *)
                LOGE "Port 80 is used by an unrelated process: ${owner}"
                LOGE "Free it, or configure Cloudflare DNS-01:"
                LOGE "  export CF_Token=<your-cloudflare-api-token>"
                LOGE "  x-ui ssl auto ${domain}"
                return 1
                ;;
        esac
    fi
    ...
    if [[ ${issued} -ne 0 ]]; then
        # A "domain already verified / cert not due" result is a success for our
        # purposes - we only need the files on disk.
        LOGW "Issue step returned non-zero; trying to install an existing certificate"
    fi

    # Reload failures make acme.sh exit non-zero even when the files were
    # written, so verify by looking at the files instead of the exit code.
    ...
    if [[ ! -s "${fullchain}" || ! -s "${keyfile}" ]]; then
        LOGE "Certificate files were not created for ${domain}"
        LOGE "Check: ${HOME}/.acme.sh/acme.sh.log"
        return 1
    fi
    ...
    LOGI "Certificate ready for ${domain}:"
    LOGI "  ${fullchain}"
    LOGI "  ${keyfile}"
    return 0
}

# Read the generated Xray config, find certificate paths that do not exist, and
# issue each one. The domain is taken from the directory name, which is the
# layout ssl_cert_issue and ssl_cert_auto both produce.
ssl_cert_fix() {
    local cfg="${xui_folder}/bin/config.json"
    if [[ ! -f "${cfg}" ]]; then
        LOGE "Xray config not found at ${cfg}"
        return 1
    fi

    # "kind|path" so each missing file can be traced to the destination the
    # inbound expects, rather than assuming /root/cert/<domain>/.
    local mapping
    mapping=$(grep -oE '"(certificateFile|keyFile)"[[:space:]]*:[[:space:]]*"[^"]+"' "${cfg}" 2> /dev/null \
        | sed -E 's/^"([a-zA-Z]+)"[[:space:]]*:[[:space:]]*"(.*)"$/\1|\2/' | sort -u)

    if [[ -z "${mapping}" ]]; then
        LOGI "No TLS certificate paths referenced by any inbound. Nothing to do."
        return 0
    fi
    ...
    if [[ -z "${missing}" ]]; then
        LOGI "Every referenced certificate file exists. Nothing to do."
        ensure_acme_cron
        return 0
    fi

    echo -e "${yellow}Missing certificate files:${plain}"
    echo "${missing}" | sed '/^$/d;s/^/  /'

    local dirs
    dirs=$(echo "${missing}" | sed '/^$/d' | xargs -r -n1 dirname | sort -u)

    local dir domain fullchain keyfile failed=0
    while IFS= read -r dir; do
        [[ -z "${dir}" ]] && continue
        domain=$(basename "${dir}")
        if ! is_domain "${domain}"; then
            LOGW "Skipping ${dir}: '${domain}' is not a domain name, cannot issue automatically."
            LOGW "Point that inbound at a real certificate, or use a self-signed one."
            failed=1
            continue
        fi
        ...
        LOGI "Issuing certificate for ${domain}..."
        ssl_cert_auto "${domain}" "${fullchain}" "${keyfile}" || failed=1
    done <<< "${dirs}"

    if [[ ${failed} -eq 0 ]]; then
        LOGI "All missing certificates issued. Restarting the panel..."
        systemctl restart x-ui 2> /dev/null || rc-service x-ui restart 2> /dev/null
        return 0
    fi
    LOGE "Some certificates could not be issued; see the messages above."
    return 1
}

ssl_cert_renew_all() {
    ensure_acme || return 1
    "$(acme_bin)" --cron --home "${HOME}/.acme.sh" || true
    ensure_acme_cron
    LOGI "Renewal pass complete."
}
...
ssl_cert_issue() {
    ...
    # Restart the unit directly instead of shelling back into this script.
    # `x-ui restart` starts a second copy of x-ui.sh, which calls systemctl
    # itself and then polls status with its own sleep - all of it invisible when
    # acme.sh output is captured, which made a slow restart look like a freeze.
    reloadCmd="systemctl restart x-ui 2>/dev/null || rc-service x-ui restart 2>/dev/null"

    LOGI "Default --reloadcmd for ACME is: ${yellow}systemctl restart x-ui"
    ...
    # install the certificate
    #
    # The output was previously swallowed by $( ), so nothing acme.sh or the
    # reload command printed reached the screen. With a slow panel restart that
    # presented as a hang with a blinking cursor and no way to tell what was
    # happening. Stream it and keep a copy for the checks below, and put a
    # ceiling on it so a wedged reload command cannot block forever.
    local installOutput="" installLog="" installRc=0
    installLog=$(mktemp)
    local -a installCmd=(~/.acme.sh/acme.sh --installcert -d "${domain}"
        --key-file "/root/cert/${domain}/privkey.pem"
        --fullchain-file "/root/cert/${domain}/fullchain.pem"
        --reloadcmd "${reloadCmd}")
    if command -v timeout > /dev/null 2>&1; then
        timeout 180 "${installCmd[@]}" 2>&1 | tee "${installLog}"
    else
        "${installCmd[@]}" 2>&1 | tee "${installLog}"
    fi
    installRc=${PIPESTATUS[0]}
    installOutput=$(cat "${installLog}" 2> /dev/null)
    rm -f "${installLog}"
    if [[ ${installRc} -eq 124 ]]; then
        LOGE "acme.sh --installcert timed out after 180s - the reload command did not return."
        LOGE "Check: systemctl status x-ui  and  journalctl -u x-ui -n 50"
    fi
...
ssl_cert_issue_CF() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # See above: never remove a working certificate before the replacement
        ...
        # Same as the domain path: restart the unit, do not re-enter this script.
        reloadCmd="systemctl restart x-ui 2>/dev/null || rc-service x-ui restart 2>/dev/null"
...
│  ${blue}x-ui netopt apply${plain}           - Apply network tuning             │
│  ${blue}x-ui netopt rollback${plain}        - Revert network tuning            │
│  ${blue}x-ui netopt status${plain}          - Show network tuning state        │
│  ${blue}x-ui diag${plain}                   - Dump active limits and counters  │
│  ${blue}x-ui diag latency${plain}           - Verify TCP low-latency path      │
│  ${blue}x-ui diag udp${plain}               - UDP loss counters and buffers    │
│  ${blue}x-ui shaper check${plain}           - Can this host shape traffic?     │
│  ${blue}x-ui shaper apply${plain}           - Enable queue-based shaping       │
│  ${blue}x-ui shaper rollback${plain}        - Remove shaping                   │
│  ${blue}x-ui ssl fix${plain}                - Issue any missing certificates   │
│  ${blue}x-ui ssl auto <domain>${plain}      - Issue one certificate, no prompts│
│  ${blue}x-ui ssl renew${plain}              - Force a renewal pass             │
│  ${blue}x-ui iplimit install${plain}        - Set up Fail2Ban IP-limit jail    │
│  ${blue}x-ui iplimit remove${plain}         - Remove Fail2Ban IP-limit jail    │
│  ${blue}x-ui shaper status${plain}          - Show shaping classes             │
└────────────────────────────────────────────────────────────────┘"
}
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "update-all-geofiles")
            ...
            ;;
        "netopt")
            network_optimize "${2:-status}"
            ;;
        "diag")
            limits_diag "${2:-full}" "${3:-}" "${4:-}"
            ;;
        "shaper")
            shaper "${2:-status}" "${3:-}"
            ;;
        "ssl")
            case "${2:-fix}" in
                auto) ssl_cert_auto "${3:-}" ;;
                fix) ssl_cert_fix ;;
                renew) ssl_cert_renew_all ;;
                *)
                    echo -e "${red}usage: x-ui ssl {fix|auto <domain>|renew}${plain}"
                    exit 1
                    ;;
            esac
            ;;
        "iplimit")
            # Non-interactive entry point for the Fail2Ban IP-limit jail, so the
            # installer can set it up without driving the menu. install_iplimit
            # exits non-zero on failure; because this runs as its own process,
            # a failure here can never abort the caller's install.
            case "${2:-install}" in
                install) install_iplimit ;;
                remove) remove_iplimit ;;
                *)
                    echo -e "${red}usage: x-ui iplimit {install|remove}${plain}"
                    exit 1
                    ;;
            esac
            ;;

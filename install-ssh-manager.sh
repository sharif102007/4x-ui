...
grn "==> 3/8 Preserving the host's kernel network settings"
# TCP_NODELAY is a per-socket option, not a Linux sysctl. Older copies of this
# installer attempted to set net.ipv4.tcp_nodelay and globally raised TCP
# buffers to 128 MiB; that could partially apply while being reported as
# skipped, and could worsen latency through bufferbloat. Network tuning is now
# explicit and reversible through `x-ui netopt apply`.
grn "    no global sysctl changes applied (optional: x-ui netopt apply)"

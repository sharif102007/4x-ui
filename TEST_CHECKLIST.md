...
## M. Automatic SSL
- [ ] `x-ui ssl fix` on the affected box: finds the missing
      `/root/cert/<domain>/fullchain.pem` and `privkey.pem`, issues them,
      restarts the panel, and Xray reaches **Running**.
- [ ] With Cloudflare creds exported (`export CF_Token=...`), `x-ui ssl auto <domain>`
      uses DNS-01 and never touches port 80.
- [ ] Without CF creds and with the SSH inbound on port 80: the panel is stopped
      for the challenge and **restarted afterwards even if acme.sh fails**
      (test by passing a domain that does not resolve — panel must come back up).
- [ ] If an unrelated service holds port 80, the command refuses instead of
      killing it, and prints the Cloudflare alternative.
- [ ] `crontab -l | grep acme` shows the renewal job afterwards.
- [ ] `x-ui ssl renew` runs a renewal pass without prompting.
- [ ] Certificate error in the panel log now ends with the
      "Run 'x-ui ssl fix'" hint.
- [ ] Existing interactive menu items 19 and 20 still work unchanged.

## N. Automatic inbound-certificate repair on update
- [ ] With every certificate present, `x-ui update` prints
      "All inbound TLS certificates are present." and changes nothing.
- [ ] Rename one cert directory to simulate loss, run `x-ui update`:
      the missing files are listed, re-issued, and Xray reaches Running
      without any prompt.
- [ ] The update **never waits for input** at this step (run it piped:
      `yes | x-ui update` must not hang).
- [ ] Kill the network mid-issue: the acme.sh call times out, the update
      finishes, and the panel is still running.
- [ ] With the SSH inbound on port 80: the panel is paused and **restarted**
      even when issuance fails.
- [ ] An unrelated service on port 80 → the step refuses and prints the
      Cloudflare alternative instead of stopping it.
- [ ] A cert path that is not a domain (self-signed / custom path) is skipped
      with a message, not mangled.
- [ ] `XUI_SKIP_CERT_REPAIR=1 x-ui update` skips the whole step.
- [ ] An existing, valid certificate is never overwritten.
- [ ] An inbound whose cert path is NOT /root/cert/<domain>/ (custom filename or
      directory) is repaired **at that exact path**.

### Already verified here (mock harness, 36 assertions, all passing)
Detection of missing vs present certificates; custom filenames and directories;
healthy certificates left untouched; non-domain paths skipped; the port-80 free /
panel-held / third-party-held branches; the Cloudflare DNS-01 path never touching
port 80; the panel always running after a failed issuance; the skip flag; a
missing config.json; and no interactive prompt anywhere in the update path.
Not verified here: real acme.sh behaviour and Go compilation.

## O. Repository hygiene (1.1.1)
- [ ] README renders on GitHub with the panel screenshot visible — the files
      were named 3x-ui-*.png while every README referenced 4x-ui-*.png, so the
      header image was broken on every language README.
- [ ] Removed as unreferenced upstream leftovers (Telegram/API tutorial images
      for docs that do not exist in this fork): botfather, newbot, token,
      user-id, panel-bot-config, APIKey1, APIKey2, DetailEnter, default-yellow,
      donation-button-black. Restore from upstream 3x-ui if ever needed.
- [ ] media/01-*..07-* screenshots of this panel are kept (unreferenced today,
      but they document this fork and are worth linking from the README).

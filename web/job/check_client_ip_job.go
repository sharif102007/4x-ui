...
var limitDependencySetup struct {
	sync.Mutex
	running     bool
...
// A concurrent-session limit is based on recently accepted sessions rather
// than the long-lived IP history used by the legacy IP-limit feature. Xray
// does not expose per-user session counters through its public API, so the
// access-log gate uses this bounded activity window.
const sessionStaleAfterSeconds = int64(45)
...
func (j *CheckClientIpJob) Run() {
	...
	sessionLimitActive := j.hasSessionLimit()
	if iplimitActive || sessionLimitActive {
		ensureLimitDependencies()
	}
	...
	if isAccessLogAvailable {
		if runtime.GOOS == "windows" {
			...
			if iplimitActive {
				if f2bInstalled || sessionLimitActive {
					shouldClearAccessLog = j.processLogFile()
				} else {
					if !f2bInstalled {
	...
	// Keep the access log while a concurrent-session gate is active. Clearing it
	// immediately after an over-limit event would erase the evidence used to
	// keep the active-session window occupied and would let the next connection
	// bypass the gate.
	if (!sessionLimitActive && shouldClearAccessLog) ||
		(!sessionLimitActive && isAccessLogAvailable && time.Now().Unix()-j.lastClear > 3600) {
		j.clearAccessLog()
	}
}

// ensureLimitDependencies performs a non-interactive, one-time bootstrap of
// Fail2Ban and its firewall dependencies. The shell helper also creates the
// 3x-ipl jail used by both legacy IP limits and concurrent-session limits.
// Installation runs asynchronously so a package-manager lock cannot stall the
// 10-second monitoring job.
func ensureLimitDependencies() {
	if runtime.GOOS == "windows" {
		return
	}

	limitDependencySetup.Lock()
	now := time.Now()
	if limitDependencySetup.running || now.Before(limitDependencySetup.nextAttempt) {
		limitDependencySetup.Unlock()
		return
	}
	limitDependencySetup.running = true
	limitDependencySetup.nextAttempt = now.Add(5 * time.Minute)
	limitDependencySetup.Unlock()

	executable, err := os.Executable()
	if err != nil {
		logger.Warningf("[LimitIP] Cannot locate x-ui helper for automatic Fail2Ban setup: %v", err)
		limitDependencySetup.Lock()
		limitDependencySetup.running = false
		limitDependencySetup.nextAttempt = time.Now().Add(30 * time.Second)
		limitDependencySetup.Unlock()
		return
	}
	scriptPath := filepath.Join(filepath.Dir(executable), "x-ui.sh")
	if _, err := os.Stat(scriptPath); err != nil {
		logger.Warningf("[LimitIP] Automatic Fail2Ban helper not found at %s", scriptPath)
		limitDependencySetup.Lock()
		limitDependencySetup.running = false
		limitDependencySetup.nextAttempt = time.Now().Add(30 * time.Second)
		limitDependencySetup.Unlock()
	...
	go func() {
		cmd := exec.Command("bash", scriptPath, "ensure-iplimit")
		output, err := cmd.CombinedOutput()
		limitDependencySetup.Lock()
		limitDependencySetup.running = false
		if err != nil {
			limitDependencySetup.nextAttempt = time.Now().Add(30 * time.Second)
			logger.Warningf("[LimitIP] Automatic Fail2Ban setup failed: %v (%s)", err, string(output))
		} else {
			limitDependencySetup.nextAttempt = time.Now().Add(24 * time.Hour)
			logger.Infof("[LimitIP] Xray access-log/Fail2Ban dependencies configured automatically")
		}
		limitDependencySetup.Unlock()
	}()
}
...
func (j *CheckClientIpJob) hasSessionLimit() bool {
	db := database.GetDB()
	var inbounds []*model.Inbound
...
func (j *CheckClientIpJob) updateInboundClientIps(inboundClientIps *model.InboundClientIps, clientEmail string, newIpsWithTime []IPWithTimestamp) bool {
	...
	if !clientFound || (limitIp <= 0 && maxSessions <= 0) || !inbound.Enable {
	...
	// A session limit is a short-lived activity gate. If both limits are set,
	// the stricter value wins; this prevents the session limit from silently
	// weakening an existing IP limit.
	effectiveLimit := limitIp
	staleAfter := ipStaleAfterSeconds
	sessionPolicy := false
	if maxSessions > 0 && (effectiveLimit <= 0 || maxSessions < effectiveLimit) {
		effectiveLimit = maxSessions
		staleAfter = sessionStaleAfterSeconds
		sessionPolicy = true
	}

	// Merge old and new IPs, evicting entries that haven't been
	// re-observed in a while. See mergeClientIps / #4077 for why.
	ipMap := mergeClientIps(oldIpsWithTime, newIpsWithTime, time.Now().Unix()-staleAfter)
	...
	// When the log has not emitted a new "accepted" line since the previous
	// pass, retain recently-seen sessions until their short activity window
	// expires. This also makes log rotation safe for the session gate.
	if sessionPolicy {
		for ip := range ipMap {
			observedThisScan[ip] = true
		}
	}
	...
	if len(liveIps) > effectiveLimit {
		...
		// The legacy IP-limit path resets the user so Xray drops the banned
		// address immediately. A concurrent-session gate must preserve the
		// sessions that already own the slots; Fail2Ban then rejects only the
		// excess source addresses from the load-bearing log line above.
		if !sessionPolicy {
	...
	if len(j.disAllowedIps) > 0 {
		logger.Infof("[SESSION_LIMIT] Client %s: kept %d active sessions, rejected %d excess sessions", clientEmail, len(keptLive), len(j.disAllowedIps))
	}

...
//     the panel and SSH are never shaped;
...
const (
	...
	shaperIFBDevice  = "ifb-4xui"
	shaperTimeout    = 90 * time.Second
)

var (
	shaperMu               sync.Mutex
	shaperStateInitialized bool
	shaperWanted           bool
	shaperApplied          bool
	shaperUnusable         bool
)

// shaperLooksInstalled recovers the persistent kernel state after an x-ui
// process restart. The helper script creates this dedicated IFB device on
// apply and removes it on rollback.
func shaperLooksInstalled() bool {
	return exec.Command("ip", "link", "show", "dev", shaperIFBDevice).Run() == nil
}
...
// policy. Repeated presence notifications are intentionally ignored: rebuilding
// a live WAN/IFB qdisc tree for an unchanged desired state can interrupt active
// connections and cause latency spikes. On the first call after a process
// restart, persistent IFB state is detected so an obsolete shaper can be
// removed when no limits are configured.
func syncTrafficShaper(wanted bool) {
	shaperMu.Lock()
	defer shaperMu.Unlock()

	if !shaperStateInitialized {
		shaperStateInitialized = true
		shaperApplied = shaperLooksInstalled()
		shaperWanted = shaperApplied
	}

	if wanted == shaperWanted && shaperApplied == wanted {
		return
	}
	shaperWanted = wanted

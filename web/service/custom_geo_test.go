...
func TestCustomGeoValidateURL(t *testing.T) {
	// URL syntax validation should not depend on external DNS availability in
	// CI or offline build environments; SSRF behavior is covered by the guard
	// itself and the download tests below.
	disableSSRFCheck(t)
	s := CustomGeoService{}

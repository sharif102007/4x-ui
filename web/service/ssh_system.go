...
func buildStunnelConfig(svcs []stunnelSvc) string {
	var sb strings.Builder
	sb.WriteString("; Managed by 4x-ui SSH Manager. Do not edit by hand.\n")
	sb.WriteString("foreground = yes\n")
	sb.WriteString("pid =\n")
	sb.WriteString("debug = 4\n")
	// stunnel owns the public TLS socket and the loopback connection to the
	// payload gateway. Disable Nagle on both so ssh_tls and ssh_tls_payload get
	// the same low-latency behavior as the in-process TCP gateway.
...
func (s sshSystem) writeStunnel(svcs []stunnelSvc) error {
	...
	if err := os.WriteFile(stunnelConfPath, []byte(buildStunnelConfig(svcs)), 0o644); err != nil {
		return err
	}

...
func (s sshSystem) applySshdPorts(ports []int, banners map[int]string) error {
	...
	// Performance: fastest ciphers first, no compression (compression adds CPU latency
	// and hurts already-compressed traffic like games/video).
	sb.WriteString("Ciphers chacha20-poly1305@openssh.com,aes128-gcm@openssh.com,aes128-ctr\n")
	sb.WriteString("MACs hmac-sha2-256-etm@openssh.com,umac-128-etm@openssh.com\n")
...
// parseExpiryInput accepts either a unix-ms integer or a YYYY-MM-DD string and
// returns unix-ms (0 = never).
func parseExpiryInput(v string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("invalid expiry (use YYYY-MM-DD): %v", err)
	}

...
func tryInstallBadvpnPackage() bool {
	if apt, err := exec.LookPath("apt-get"); err == nil {
		...
		// when UDPGW is requested and missing, never on normal panel startup.
		_, _ = runUdpSetup(3*time.Minute, apt, "update", "-y")
		if _, err := runUdpSetup(3*time.Minute, apt, "install", "-y", "badvpn"); err == nil && BadvpnInstalled() {
			return true
		}
	}
...
func installBadvpnBuildDeps() error {
	...
	case toolExists("apt-get"):
		cmd = "apt-get"
		args = []string{"install", "-y", "build-essential", "cmake", "git"}
	case toolExists("dnf"):
		cmd = "dnf"
...
func reconcileSystemdUdpRelays(desired map[int]struct{}) error {
	...
	// Bring every desired listener up before removing old ones. If startup
	// fails, clean only newly-created units and leave the prior working state.
	for _, port := range sortedPorts(desired) {
		...
		if out, err := sys.run("systemctl", "enable", unit); err != nil {
			return fmt.Errorf("enable %s: %v: %s", unit, err, strings.TrimSpace(out))
		}
		active := unitActive(sys, unit)
		var out string
		if changed && active {
			out, err = sys.run("systemctl", "restart", unit)
		} else if !active {
			out, err = sys.run("systemctl", "start", unit)
		}
		if err != nil {
			if !wasOld {
			...
			return fmt.Errorf("start %s: %v: %s%s", unit, err, strings.TrimSpace(out), func() string {
				if logTail == "" {
					return ""
				}
				return "\n" + logTail

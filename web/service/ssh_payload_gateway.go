...
// tcpNoDelaySetter is implemented by *net.TCPConn. Keeping the helper behind
// this small interface makes the intent explicit and testable while allowing
// non-TCP net.Conn implementations used by tests to pass through unchanged.
type tcpNoDelaySetter interface {
	SetNoDelay(bool) error
}

// enableTCPNoDelay disables Nagle's algorithm on a TCP connection. Go normally
// enables TCP_NODELAY on TCP sockets already, but the payload gateway is the
// latency-sensitive extra hop added by 4x-ui, so enforce it explicitly on both
// sides rather than depending on a runtime default. TCP_NODELAY is a socket
// option; there is no net.ipv4.tcp_nodelay sysctl.
func enableTCPNoDelay(conn net.Conn) error {
	if conn == nil {
		return fmt.Errorf("cannot enable TCP_NODELAY on a nil connection")
	}
	setter, ok := conn.(tcpNoDelaySetter)
	if !ok {
		return nil
	}
	return setter.SetNoDelay(true)
}
...
func (g *payloadGateway) acceptLoop() {
	...
	for {
		...
		if err := enableTCPNoDelay(conn); err != nil {
			// A socket-option failure should be visible but must not turn a
			// working SSH connection into an outage.
			logger.Warningf("ssh-manager: gateway #%d client TCP_NODELAY: %v", g.id, err)
		}
		g.wg.Add(1)
		go func() {
			defer g.wg.Done()
...
func (g *payloadGateway) handle(client net.Conn) {
	...
	defer backend.Close()
	if err := enableTCPNoDelay(backend); err != nil {
		logger.Warningf("ssh-manager: gateway #%d backend TCP_NODELAY: %v", g.id, err)
	}

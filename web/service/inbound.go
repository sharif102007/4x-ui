...
func (s *InboundService) AddInboundClient(data *model.Inbound) (bool, error) {
	...
	for _, client := range clients {
		if client.MaxSessions < 0 || client.MaxSessions > 10000 {
			return false, common.NewError("concurrent session limit must be between 0 and 10000")
		}
	}
	...
	if !apiReady {
		// Persist the client and let the normal Xray restart reconcile the full
		// configuration. Most importantly, never leave the HTTP request waiting
		// forever when Xray's gRPC API is temporarily unavailable.
		logger.Warning("Xray API unavailable while adding client; scheduling config restart")
		needRestart = true
	}
	for _, client := range clients {
		...
		for _, client := range clients {
			if len(client.Email) > 0 && client.Enable {
				...
				if err1 == nil {
					...
					logger.Warning("Error in adding client by api:", err1)
					needRestart = true
				}
			}
		}
	...
	if clientIndex >= 0 && clientIndex < len(settingsClients) {
		if oldMap, ok := settingsClients[clientIndex].(map[string]any); ok {
			...
			if newMap, isNewMap := interfaceClients[0].(map[string]any); isNewMap {
				for _, key := range []string{"speedLimit", "downloadMbps", "uploadMbps", "maxSessions"} {

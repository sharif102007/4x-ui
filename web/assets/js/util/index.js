...
class HttpUtil {
    ...
    static async postWithModal(url, data, modal) {
        ...
        // Modal actions must always settle. A stalled backend/Xray request
        // should show an error instead of leaving the form permanently
        // disabled with a spinner.
        const msg = await this.post(url, data, { timeout: 15000 });
        if (modal) {
            modal.loading(false);

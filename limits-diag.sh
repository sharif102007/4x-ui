...
STUNNEL_CONF="/etc/stunnel/xui-ssh-manager.conf"
...
show_tcp_latency() {
    hdr "TCP low-latency path"
    echo "  payload gateway: TCP_NODELAY is enforced on accepted and backend TCP sockets"
    echo "  OpenSSH:         TCPKeepAlive enabled; SSH keepalive interval is 30 seconds"
    if [[ -f "${STUNNEL_CONF}" ]]; then
        if grep -Fxq "socket = l:TCP_NODELAY=1" "${STUNNEL_CONF}" &&
           grep -Fxq "socket = r:TCP_NODELAY=1" "${STUNNEL_CONF}"; then
            echo "  stunnel:         TCP_NODELAY enabled on public and backend sockets"
        else
            warn "  stunnel: config exists without both TCP_NODELAY entries"
            warn "           restart x-ui to reconcile ${STUNNEL_CONF}"
        fi
    else
        echo "  stunnel:         not configured (normal when no TLS SSH inbound is enabled)"
    fi
    echo "  sysctl:          TCP_NODELAY is per-socket; net.ipv4.tcp_nodelay is not a Linux key"
    ...
    echo
    echo "  High latency only while downloading/uploading is queueing (bufferbloat),"
    echo "  not proof that TCP_NODELAY is off. Check 'x-ui shaper status' and tc state."
}
...
show_udp_counters() {
    ...
    if have ss; then
        local listeners
        listeners=$(ss -lunp 2>/dev/null | grep -i udpgw || true)
        if [[ -n "${listeners}" ]]; then
            echo "${listeners}" | sed 's/^/  /'
        else
            warn "  no udpgw listener found"
        fi
    fi
    if have ps; then
...
case "${1:-full}" in
    ...
    full) do_full ;;
    latency) show_tcp_latency ;;
    udp) do_udp "${2:-}" "${3:-10}" ;;
    *)
        echo "usage: $0 {full|counters|latency|udp [peer-host] [seconds]}"
        exit 1
        ;;
esac

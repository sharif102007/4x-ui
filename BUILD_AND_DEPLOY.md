...
tests first. Only after analysis succeeds does it build the single Linux
`amd64` package and Windows package. The Linux asset supports x86_64 Debian,
Ubuntu, and AlmaLinux hosts. A version tag such as `v1.1.9` uploads the archives to the
...
- `x-ui` and the `xray-linux-amd64` binary
- `x-ui.sh`
...
HTML, JavaScript, CSS, and translations are embedded in the Go binary. The

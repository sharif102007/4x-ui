...
The runtime changes the live queue tree only when its desired enabled/disabled
state changes. It also detects an existing `ifb-4xui` device after a process
restart, while the installer and updater roll back an old queue tree before
replacing the panel. This avoids stale shaping and unnecessary connection or
latency interruptions during upgrades.
...
`net.ipv4.tcp_nodelay` sysctl: Go manages its TCP socket defaults, while Xray
and stunnel manage their own sockets.

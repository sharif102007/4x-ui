...
- One nftables table read collects every user's upload/download counters every
  ten seconds; it does not start a process per user.
...
- A signature and live health check preserve the current qdisc when rates have
  not changed.
- Once HTB/IFB is healthy, nftables drop-policer rules are removed atomically;
  marking and named traffic counters remain active.
...
`net.ipv4.tcp_nodelay` sysctl. The SSH payload gateway explicitly applies
`TCP_NODELAY`; the generated stunnel configuration applies it to both accepted
and backend sockets, while Xray manages its own sockets.

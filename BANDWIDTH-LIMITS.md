...
equivalent. Nothing in this codebase sets it explicitly: Go's `net` package
enables it by default on every TCP connection it creates or accepts, so the
panel's sockets already have Nagle disabled. Xray manages its own sockets and
is not configured from here.

The panel also sets a soft GC memory limit at startup (75% of the cgroup limit,
or of host RAM when uncapped). Override with `XUI_MEMORY_LIMIT=<bytes>` in the
environment file, or `XUI_MEMORY_LIMIT=0` to keep stock Go behaviour.
